...
class FlashAttentionPytorch(torch.autograd.Function):
	...
	def forward(ctx, Q, K, V, is_causal=False):
		...
		for i in range(num_iters):
			...
			l_i = torch.zeros(batch_size, Q_i.shape[1], device=Q.device, dtype=torch.float32)   # (B, Ti)
			m_i = torch.full((batch_size, Q_i.shape[1]), -float('inf'), device=Q.device, dtype=Q.dtype)  # (B, Ti)
			...
			for j in range(num_iters):
				...
				S = (Q_i @ K_j.transpose(-2, -1)) / math.sqrt(dim)  # (B, Ti, Tj) in input dtype
				old_mi = m_i                                          # (B, Ti)
				m_i = torch.max(m_i, torch.max(S, dim=-1).values)   # (B, Ti) float32
				P = torch.exp(S - m_i.unsqueeze(-1))        # (B, Ti, Tj) float32

				correction = torch.exp(old_mi - m_i)                 # (B, Ti) float32
				l_i = correction.float() * l_i + P.float().sum(dim=-1)               # (B, Ti) float32
				O_i = correction.unsqueeze(-1) * O_i + P @ V_j  # (B, Ti, d) input dtype

			O[:, i_start: i_start + tile_size] = (1 / l_i).unsqueeze(-1).to(Q.dtype) * O_i
			L[:, i_start: i_start + tile_size] = m_i.float() + torch.log(l_i)
